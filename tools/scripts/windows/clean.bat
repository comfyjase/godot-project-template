...
set "godot_file_suffix=%configuration%"
if "%configuration%" == "template_release" (
	set "godot_file_suffix=%godot_configuration%"
) else (
	set "godot_file_suffix=%godot_configuration%.dev"
)
...
cd godot-cpp
del /S *.windows.%godot_file_suffix%.x86_64.obj
del /S *.windows.%godot_file_suffix%.x86_64.lib
if %errorlevel% neq 0 exit /b %errorlevel%

...
print("==========================================", flush=True)
URL = "https://github.com/AccessKit/accesskit-c/releases/download/0.17.0/accesskit-c-0.17.0.zip"
r = requests.get(URL)
with open("accesskit-c-0.17.0.zip", "wb") as fd:
    fd.write(r.content)
with zipfile.ZipFile("accesskit-c-0.17.0.zip") as archive:
    for file in archive.namelist():
        if file.startswith('accesskit-c-0.17.0'):
...
if (os.getenv("JAVA_HOME") is None and os.getenv("ANDROID_HOME") is None) or not os.path.exists(f"{Path.home().drive}/sdks/android/platform-tools"):
    ...
    android_sdk_version_number = "35.0.0"
    android_ndk_version_number = "28.1.13356709"
    android_cmake_version_number = "3.10.2.4988404"
    ...
    run_subprocess(f"{android_sdk_root_folder}/cmdline-tools/latest/bin/sdkmanager.bat --sdk_root={android_sdk_root_folder} \"platform-tools\" \"build-tools;{android_sdk_version_number}\" \"platforms;android-{android_sdk_version}\" \"cmdline-tools;latest\" \"cmake;{android_cmake_version_number}\" \"ndk;{android_ndk_version_number}\"", f"Failed to install android sdk {android_sdk_version_folder_name}")
    print("Done", flush=True)
    print("", flush=True)
...
if (os.getenv("SCONS_CACHE") is None or os.environ["SCONS_CACHE"] != project_scons_cache_path):
